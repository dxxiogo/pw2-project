...
import { loginRestaurant } from "@/store/restaurantAuthSlice.ts";
...
async function handleLogin(e: React.FormEvent) {
  e.preventDefault();
  const result = await dispatch(loginRestaurant({ email, password }));

  if (loginRestaurant.fulfilled.match(result)) {
    localStorage.setItem("restaurant", JSON.stringify(result.payload));
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 items-center">
      <div className="flex flex-col justify-center items-start w-full md:w-1/2 px-8">
        ...
        />
        <h1 className="text-3xl font-bold mb-4 mt-4">Login Restaurante</h1>
